...
// 	convertPointsToCluster(groundtruth_points_filtered , clusters_GTs);
	...
	vector<ClusterPtr> clusters_GTs;
	convertPointsToCluster( groundtruth_small_points_removed_sorted , clusters_GTs );
	
	vector<ClusterPtr> clusters;
 	double threshold = 2;  //[m]		
 	simpleClustering(groundtruth_small_points_removed ,threshold , clusters); 
	
	vector<ClusterPtr> clusters_Premebida;
	double threshold_cosine = 0.70;
	premebidaClustering(groundtruth_small_points_removed ,threshold_cosine , clusters_Premebida);
	
	vector<ClusterPtr>  clusters_Dietmayer;
	double C0 = 1.5;
	dietmayerClustering(groundtruth_small_points_removed, C0, clusters_Dietmayer);
	
	vector<ClusterPtr> clusters_Santos;
	double Beta = deg2rad(30.0);
	santosClustering(groundtruth_small_points_removed , C0 , Beta , clusters_Santos);
	
	vector<ClusterPtr> clusters_ABD;
	double lambda =  deg2rad(5.0);
	abdClustering(groundtruth_small_points_removed ,lambda, clusters_ABD);
	
	vector<ClusterPtr> clusters_nn;
	double threshold_nn = 1.5;
	nnClustering( groundtruth_small_points_removed , threshold_nn , clusters_nn);
	
// 	Vizualize the Segmentation results
	visualization_msgs::MarkerArray targets_markers;
   	targets_markers.markers = createTargetMarkers( clusters , clusters_Premebida , clusters_Dietmayer, clusters_Santos ,clusters_ABD, clusters_nn, clusters_GTs );	
